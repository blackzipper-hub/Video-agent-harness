...
cuti-video-agent
  ...
  .github/workflows/vda-images.yml
  .github/workflows/vda-deploy.yml
...
GitHub **不在 push 时 helm**。DeepSeek 原来的 `ci.yml` / `e2e.yml` **不要删**。我们的是：

| 仓 | 文件 | 干什么 |
|----|------|--------|
| cuti-video-agent | `Build images` | 推代码（或手动 Run）把 Studio / Runtime / Media / DSH 编进 ECR |
| cuti-video-agent | `Deploy` | **手动** Run：选 dev 或 prod，勾要上的服务（默认全勾） |
| Cuti-backend-go | `Build Go image` | 把 Go API 编进 ECR |
| Cuti-backend-go | `Deploy Go` | **手动** Run：只上 Go |

日常跟 EC2 Deployment Console 同一思路：选 **dev** 或 **prod** → 勾服务 → Run。不用选手动分支。镜像 tag 写死：**dev 环境 = `dev`，prod 环境 = `main`**。两个仓要分别点：**Deploy**（Studio / Runtime / Media / DSH）和 **Deploy Go**。prod 建议 GitHub Environment `vda-prod` 开 required reviewers。GitHub 托管 runner 需要仓库 Secret `AWS_ROLE_ARN`（OIDC）；EC2 上的 instance role 只给那台机器用，GitHub 的虚机用不了。没配好之前镜像和 helm 仍走跳板机。GitHub Deploy 调用的是 `deploy/vda-upgrade.sh`，**不会**跑 `vda-apply.sh`（那是第一次装 namespace 用的，不要删）。
