...
Use the persisted transcript as the only source of dialogue, never the screenplay or
generation prompt. Inspect its segments before rendering. If it misses audible speech,
repeat transcription of the original audio without an invented dialogue prompt.
For requested translation, pass `translated_texts`: exactly one translated string per
transcript segment in the same order. The renderer preserves its timestamps. Never
pass caption_html, composition_html, replacement cues or invented timestamps for dialogue.
The Cuti template owns placement, font and timing; change style, position and accent_color
to edit appearance by conversation. Render from the original pre-caption video.
Preview and apply the requested operations without another confirmation. Transcription
alone is not completion: inspect its result, translate if requested, then render.
