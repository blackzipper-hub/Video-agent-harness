...
镜头、风格、导演锚点、作画术语。H3 / 即梦写 prompt 都从这里取词。按需查阅，不必通读。
...
举例：这类主题里，词可以这样配。气质从这次 brief / 歌 / 源来，不必整格搬进片子。

| 主题 | 词可以这样配 |
|------|----------|
| 仙侠/修真 | 3D 国漫渲染、中国仙侠概念设计、中国风水墨、厚涂风 |
| 古风/历史 | 水墨画、工笔画、古典绘画、胶片质感 |
| 赛博朋克/科幻 | 写实 CG、概念设计、赛博朋克霓虹、蒸汽波 |
| 现实/人物 | 电影摄影写实、纪录片、胶片颗粒、黄金时段光线 |
| 美食 | 美食广告摄影 |
| 自然风光 | 风光摄影、航拍纪录片 |
| 动漫 | 厚涂风、3D 国漫 CG、浮世绘风、超精细 CG 动画、日漫赛璐璐、中国奇幻动画电影 |
| MV/卡点 | 电影感、2.35:1 宽银幕、胶片颗粒、黄金时段光线、莫兰迪色系、红金高饱和、中国风水墨、蒸汽波、赛博朋克霓虹 |
...
提示词开头写画幅/帧率/时长/色调总纲。下面几句只示范句式，色调按这次 brief 自己写：

```
2.35:1，24fps，15秒，黄金时段光线
2.35:1，24fps，8秒，胶片颗粒低饱和度
2.35:1，24fps，15秒，莫兰迪色系
2.35:1，24fps，12秒，红金高饱和
...
即梦不要传 `audios` / `@音频`。这一段走哪档、要不要对口型，按 [SKILL.md](SKILL.md) 的〈词和画〉定，再取下面对应的句式。

**朝镜头唱的段**——抄词做口型：

```
2.35:1，24fps，N秒。定妆参考 @图片1。
严格对口型（禁止改词、禁止加词）：
「……该段 segments[i].lyrics……」
0–2秒：近景开口（口型跟上面唱词）；
2–5秒：灯光随重拍闪切。
禁止画面内文字。
```

**人在故事里的段**——不抄词，不开口唱，动作和光跟拍点走：

```
2.35:1，24fps，N秒。定妆参考 @图片1。
角色在场景里做事，不看镜头、不做唱歌口型。
0–3秒：<动作/事件>；
3–8秒：<反应或揭示>，剪点落在重拍上。
禁止画面内文字。
```

照着词演那一档就把词里的东西写成画面里的实物；推开一层就写它的更大意思；故意错开就让画面跟词各走一路。

H3 用 `audios` 传该段音频。对口型的段在 prompt 里抄该段唱词；不对口型的段只写动作和拍点，音频仍然传，用来卡拍。
...
- 区分「参考」与「编辑」
- 视频延长（词库句式）：`将@视频1延长Xs`，生成时长选**新增部分**的时长。**MV 成片不要用它当主路径**，按 [SKILL.md](SKILL.md) 切段音频再等长生成
- 多段融合：`在@视频1和@视频2之间加一个场景，内容为xxx`
- 即梦口型不要 `@音频1`，用上面〈表演模式〉里的句式；H3 用 `audios` 传段音频
...
**MV/卡点**：2.35:1、24fps；分镜写场景/动作/音效；多图卡点参考视频节奏；下方留歌词安全区。色调、作画从上面词库按这次 brief 取
...
示范完整镜头怎么写。气质跟这次歌和 brief，不必和例句同一条。
...
```
2.35:1，24fps，8秒，黄金时段光线。黄昏海边公路，侧逆光把发丝和车漆打出轮廓，浅景深；0–3秒跟拍车窗里的人看向远处，3–8秒拉远露出整段金色海平线，剪点落在鼓点上。禁止画面内文字。
```

```
16:9，24fps，5秒，宫崎骏风格。夏草坡，手绘质感，柔和自然光；少女逆风跑过，衣摆和草浪一起翻，远处一架纸飞机慢慢升起来。禁止画面内文字。
```

```
2.35:1，24fps，10秒，王家卫风格。雨后通勤走廊，手持、高饱和、抽帧；人走过一排暖色灯箱，慢动作回头，焦点从脸移到湿地面的倒影。禁止画面内文字。
```
...
MV 按 [SKILL.md](SKILL.md) 切段生成，不要把「将@视频1延长」当主路径。

下面是即梦词库里的延长句式，仅作写法参考：
