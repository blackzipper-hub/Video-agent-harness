...
|---|---|
| 锚 | 用户已经给的文本和源媒体 |
| 身份 | 源里要连续认出的主体：人、动物、当主角的物。没看见人就不要编一个人 |
| 场 / 物 | 需要锁的空间或物件；不需要就不要为流程再造一张 |
| 唱 | 有人声、且画面里真有人在唱，才写；不要默认有人 |
| 约束 | 看源事实、research、听分析、设定图、出画都站在这一边 |

用户给了源，就是它。没给，导演定一次，定完也是锁。下游给办法和气质，不换主体种类，也不换已经看见的外形。

## 能力与工具

- **看源**：先读用户文本和源媒体，写下可见事实（主体种类和数量、颜色/外形、场、道具）。没看见的属性不要编。`research.generate` 看不见图。
- **创意构思**：从源事实或一首歌发散多个创意方向，挑最有记忆点的展开
- **文案扩写**：把模糊需求扩成完整中文提示词，融入运镜/光影/节奏/风格
- **web_search**：搜当下流行 prompt 写法，借鉴句式融入文案
- **参考调研**：可见事实写成字之后，才用 `research.generate` 搜外部参考（方法见 `video_skill_load("video-research")`）。brief 必须点名已锁主体，禁止写「基于图片内容」。三条里走质量和效果更好的那条。用户明确不要搜，就跳过。
...
1. **看源**：主体是人、一群人、动物、物，还是场？各有几份？颜色、外形、道具、空间写下来。没看见的不要编。
2. **听歌定调**：有音频时，先听歌，用段落**情绪**决定哪条创意路线，再按〈词和画〉给每段定档；整张歌词纸不是分镜大纲
3. **发散创意方向**：从已经写下的源事实出发，构思 2–3 个完全不同的角度，再挑最有意思的展开。方向改气质和结构，不改主体种类。
...
- 要不要先看源写下可见事实？文案够不够具体？
- 只有源没有文案？→ 进入创意发散模式
- 可见事实写成字了，要不要再搜参考？用户明确不要搜，就跳过。
...
对着已经写成字的源事实、用户的话、歌去搜。用户明确不要搜，就跳过。方法论见 `video_skill_load("video-research")`。`research.generate` 的 `user_input` 必须点名已锁主体，不要写「基于图片内容」。
...
1. **先看源，再决定要不要搜。** 写下可见事实。然后才 `research.generate`（可跳过）。三条里走质量和效果更好的那条，后面设定图和每段 prompt 用上。调研只改气质、光、剪、音乐，不改主体种类。
2. **先有歌。** 用户已经丢了音频，直接听它。
...
3. **设定图按需。** 给每份源标角色：`identity` / `setting` / `object` / `style` / 这次不用。片子要锁什么，才用 `atomic.image.generate` 出什么（身份、场、物都可以；动物也是身份）。prompt 用看源事实 + 用户文本；没看见的属性不要编。源必须写进 `reference_from_steps` 或 `images`。`depends_on` 只排队。选中方向给气质，不换主体。
...
- 没看源、或 brief 只写「基于图片内容」，就跑 `research.generate`
- 设定图 `depends_on` 了源，却不写 `reference_from_steps` / `images`
- 出画只带生成设定图、丢掉仍要锁的上传源
