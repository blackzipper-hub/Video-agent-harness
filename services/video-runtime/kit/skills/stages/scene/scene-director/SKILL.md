...
name: scene-director
description: >-
  Cinematic scene plan: hero frames, 5-aspect craft, overlays, transitions.
---
...
|-------|----------|---------|
| Schema | `/kit/schemas/artifacts/scene_plan.schema.json` | Artifact validation |
...
>
> See `/kit/skills/creative/video-gen-prompting/SKILL.md` for the primitive vocabulary.
...
## Cuti Scene Persistence (binding)

1. Bind video-driven scenes 1:1 to script sections; `additional_data.script_section_id` must match section `id`.
2. Put 5-aspect craft + hero hints in `additional_data` — never dump `SUBJECT:`/`CAMERA:` blocks into user-facing `description`.
3. `description` = 1–3 visual sentences only.
4. Activate `/kit/skills/creative/video-gen-prompting` for vocabulary.
5. Persist with `write_scenes_artifact`. Fix validation in the same turn.
