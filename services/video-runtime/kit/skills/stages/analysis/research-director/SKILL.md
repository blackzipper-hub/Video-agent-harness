...
description: >-
  Cinematic research: visual/sound/camera batches; gather only, no final lock.
---
...
|-------|----------|---------|
| Schema | `/kit/schemas/artifacts/research_brief.schema.json` | Artifact validation |
...
Validate against `/kit/schemas/artifacts/research_brief.schema.json` before submitting.
...
## Cuti Research Persistence (binding)

You do not write a standalone research_brief file. Fill the `research` argument of `write_analysis_artifact` (→ `extra.research`).

Minimum fields the write tool requires: `topic`, `research_date`, `landscape`, ≥2 `angles_discovered` (prefer ≥3 with diversity checklist above), ≥1 `visual_references` objects, ≥1 `sources` (prefer ≥5).

Put richer OM fields (`type`, `audio_direction`, `motion_commitment`, `research_summary`, etc.) inside `research` as well — they are kept.
