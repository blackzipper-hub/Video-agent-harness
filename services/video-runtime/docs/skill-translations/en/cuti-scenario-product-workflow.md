# cuti-scenario-product-workflow: English Reading Copy

Reference translation for developers. **Documentation only; not a runtime Skill or installable bundle.**

Source: [original SKILL.md](../../../skills/external/cuti-scenario-product-workflow/SKILL.md). Source SHA-256: `a4a80dee97fb37a3a9f8601603322eb860771f9f6902eaaf9e8856524db33522`.
...
Choose one primary selling point with the strongest dramatic potential. The output must not include a `Product Truth`, `unsupported claims`, unknown-facts, or claim-audit section. Avoid reducing the product to appearance alone when its category supports a credible functional or emotional benefit.
...
Before designing the scenario, read [references/story-structures.md](../../../skills/external/cuti-scenario-product-workflow/references/story-structures.md). Select one primary story structure based on the product benefit, emotional tone, audience, duration, and requested genre. Do not combine every framework or expose theory for its own sake.
...
- an opening declaration that this is a `product advertisement` rather than a standalone drama scene;
...
```text
Task type: Product advertisement
Core product selling point: <the same primary selling point used by the whole advertisement>
This segment's selling-point evidence: <product action -> visible/audible evidence -> changed story outcome>
Shot creation requirements: Plot, performance, sound, and cinematography must jointly reinforce that selling point. Do not reduce the product to an irrelevant prop or make the segment an ordinary drama unrelated to the product.
```
