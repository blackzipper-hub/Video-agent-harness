"""Integration: Seedance 2.0 Image-to-Video via WaveSpeed (1 call, consumes credits)."""
import os
...
def test_seedance_2_i2v_cost_720p_5s():
    cost = ToolService.calculate_cost(ToolType.SEEDANCE_2_I2V, duration=5, resolution=Resolution.P720)
    assert cost == pytest.approx(1.20)


@pytest.mark.asyncio
async def test_seedance_2_i2v_smoke_wavespeed_api():
    ...
    result = await svc.generate_seedance_2_i2v(
        image=EXAMPLE_START,
