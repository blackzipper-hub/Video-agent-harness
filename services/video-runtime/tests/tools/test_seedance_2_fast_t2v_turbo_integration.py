"""Integration: Seedance 2.0 Fast Text-to-Video Turbo via WaveSpeed.
...
运行：
    pytest tests/tools/test_seedance_2_fast_t2v_turbo_integration.py -v            # 全部（真实测试无 key 自动跳过）
    pytest tests/tools/test_seedance_2_fast_t2v_turbo_integration.py -k cost -v    # 仅离线计费
    pytest tests/tools/test_seedance_2_fast_t2v_turbo_integration.py -k real -v -s # 真实并发
"""
import os
import asyncio
import random
...
# 无 reference_videos：720p $0.60/5s，1080p $0.65/5s，按输出时长线性
_NO_REF_COST_CASES = [
    (Resolution.P720, 5, 0.60),
    (Resolution.P720, 10, 1.20),
    (Resolution.P720, 15, 1.80),
    (Resolution.P1080, 5, 0.65),
    (Resolution.P1080, 10, 1.30),
    (Resolution.P1080, 15, 1.95),
]


@pytest.mark.parametrize("resolution,duration,expected", _NO_REF_COST_CASES)
def test_seedance_2_fast_t2v_turbo_cost_no_ref_video(resolution, duration, expected):
    cost = ToolService.calculate_cost(ToolType.SEEDANCE_2_FAST_T2V_TURBO, duration=duration, resolution=resolution)
    assert cost == pytest.approx(expected), f"{resolution.value} {duration}s 期望 ${expected}, 实际 ${cost}"


# 有 reference_videos：720p $1.10/5s，1080p $1.15/5s（与参考视频长度无关，仅看输出时长）
_REF_VIDEO_COST_CASES = [
    (Resolution.P720, 5, 5.0, 1.10),
    (Resolution.P720, 10, 5.0, 2.20),
    (Resolution.P720, 15, 5.0, 3.30),
    (Resolution.P1080, 5, 5.0, 1.15),
    (Resolution.P1080, 10, 5.0, 2.30),
    (Resolution.P1080, 15, 5.0, 3.45),
    # 参考视频长度不影响价格（仍按输出时长）：ref 20s、输出 5s @720p = $1.10
    (Resolution.P720, 5, 20.0, 1.10),
]


@pytest.mark.parametrize("resolution,duration,ref_dur,expected", _REF_VIDEO_COST_CASES)
def test_seedance_2_fast_t2v_turbo_cost_with_ref_video(resolution, duration, ref_dur, expected):
...
def test_seedance_2_fast_t2v_turbo_cost_480p_billed_as_720p():
    # Turbo 不支持 480p：计价按 720p
    assert ToolService.calculate_cost(ToolType.SEEDANCE_2_FAST_T2V_TURBO, duration=5, resolution=Resolution.P480) == pytest.approx(0.60)


def test_seedance_2_fast_t2v_turbo_cost_duration_clamped():
    # 3s 应被 clamp 到 4s，17s 应被 clamp 到 15s
    assert ToolService.calculate_cost(ToolType.SEEDANCE_2_FAST_T2V_TURBO, duration=3, resolution=Resolution.P720) == pytest.approx(0.60 * (4 / 5.0))
    assert ToolService.calculate_cost(ToolType.SEEDANCE_2_FAST_T2V_TURBO, duration=17, resolution=Resolution.P720) == pytest.approx(1.80)


def test_seedance_2_fast_t2v_turbo_cheaper_than_standard_turbo():
    # Fast Turbo 应比标准 T2V Turbo 便宜（相同参数）
    for res in (Resolution.P720, Resolution.P1080):
        for dur in (5, 10, 15):
            fast = ToolService.calculate_cost(ToolType.SEEDANCE_2_FAST_T2V_TURBO, duration=dur, resolution=res)
            std = ToolService.calculate_cost(ToolType.SEEDANCE_2_T2V_TURBO, duration=dur, resolution=res)
            assert fast < std, f"{res.value} {dur}s: fast ${fast} 应 < standard ${std}"
...
_SUBJECTS = ["a businesswoman power-walking through a lobby", "a knight pulling a sword from ice", "a dancer mid-spin", "waves crashing on rocks"]
...
async def _run_one(svc: WaveSpeedService, combo: dict) -> dict:
    ...
    start = time.time()
    try:
        result = await svc.generate_seedance_2_fast_t2v_turbo(
...
@pytest.mark.asyncio
async def test_seedance_2_fast_t2v_turbo_real_concurrent():
    ...
    print("\n" + "=" * 80)
    print(f"🚀 Seedance 2.0 Fast T2V Turbo 真实并发测试，组合数: {len(_REAL_COMBOS)}（均 4s，主要 720p）")
    print("=" * 80)
    ...
    assert len(succeeded) > 0, "所有 Fast T2V Turbo 组合均失败"
...
@pytest.mark.asyncio
async def test_seedance_2_fast_t2v_turbo_real_smoke():
    ...
    if not svc.api_key:
        pytest.skip("WaveSpeed API key not configured")
    result = await svc.generate_seedance_2_fast_t2v_turbo(
        prompt="A businesswoman power-walks through a corporate lobby; tracking shot; Wes Anderson symmetry, pastel palette.",
        duration=4,
        resolution="720p",
        aspect_ratio="16:9",
        generate_audio=True,
