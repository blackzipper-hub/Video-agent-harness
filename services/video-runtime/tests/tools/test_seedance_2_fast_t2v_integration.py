"""Integration: Seedance 2.0 Fast Text-to-Video via WaveSpeed.

包含两类测试：
  1) 离线计费测试（不调用 API，不扣费）：覆盖官方定价表全部组合
     - 无 reference_videos：480p $0.50/5s，720p 2×，1080p 5×
     - 有 reference_videos：按 Fast Video-Edit 每秒计费（input+output），480p/720p/1080p = 0.065/0.13/0.325
  2) 真实 API 测试（需 WAVESPEED_API_KEY，会扣费）：并发跑多组参数组合（分辨率/比例/音频/参考图），
     用最短时长 + 最便宜分辨率为主以控制成本。

运行：
    pytest tests/tools/test_seedance_2_fast_t2v_integration.py -v            # 全部（真实测试会因无 key 自动跳过）
    pytest tests/tools/test_seedance_2_fast_t2v_integration.py -k cost -v    # 仅离线计费
    pytest tests/tools/test_seedance_2_fast_t2v_integration.py -k real -v -s # 真实并发
"""
import os
import asyncio
import random
...
# 无 reference_videos：480p $0.50/5s，720p 2×，1080p 5×，按秒线性
_NO_REF_COST_CASES = [
    (Resolution.P480, 5, 0.50),
    (Resolution.P480, 10, 1.00),
    (Resolution.P480, 15, 1.50),
    (Resolution.P720, 5, 1.00),
    (Resolution.P720, 10, 2.00),
    (Resolution.P720, 15, 3.00),
    (Resolution.P1080, 5, 2.50),
    (Resolution.P1080, 10, 5.00),
    (Resolution.P1080, 15, 7.50),
]


@pytest.mark.parametrize("resolution,duration,expected", _NO_REF_COST_CASES)
def test_seedance_2_fast_t2v_cost_no_ref_video(resolution, duration, expected):
    cost = ToolService.calculate_cost(ToolType.SEEDANCE_2_FAST_T2V, duration=duration, resolution=resolution)
...
_REF_VIDEO_COST_CASES = [
    (Resolution.P480, 5, 5.0, 0.65),
    (Resolution.P720, 5, 5.0, 1.30),
    (Resolution.P1080, 5, 5.0, 3.25),
    # input 时长 clamp 到 2–15：参考 1s 视作 2s，输出 4s → (2+4)
    (Resolution.P480, 4, 1.0, 0.065 * (2 + 4)),
    (Resolution.P720, 10, 20.0, 0.13 * (15 + 10)),  # 参考 20s clamp 到 15s
]


@pytest.mark.parametrize("resolution,duration,ref_dur,expected", _REF_VIDEO_COST_CASES)
def test_seedance_2_fast_t2v_cost_with_ref_video(resolution, duration, ref_dur, expected):
...
def test_seedance_2_fast_t2v_cost_duration_clamped():
    # 3s 应被 clamp 到 4s，17s 应被 clamp 到 15s
    assert ToolService.calculate_cost(ToolType.SEEDANCE_2_FAST_T2V, duration=3, resolution=Resolution.P480) == pytest.approx(0.50 * (4 / 5.0))
    assert ToolService.calculate_cost(ToolType.SEEDANCE_2_FAST_T2V, duration=17, resolution=Resolution.P480) == pytest.approx(1.50)


def test_seedance_2_fast_t2v_cheaper_than_standard_t2v():
    # Fast 同档应比标准 T2V 便宜
    for res in (Resolution.P480, Resolution.P720, Resolution.P1080):
        fast = ToolService.calculate_cost(ToolType.SEEDANCE_2_FAST_T2V, duration=5, resolution=res)
        std = ToolService.calculate_cost(ToolType.SEEDANCE_2_T2V, duration=5, resolution=res)
        assert fast < std, f"{res.value}: fast ${fast} 应 < 标准 ${std}"
...
async def _run_one(svc: WaveSpeedService, combo: dict) -> dict:
    ...
    start = time.time()
    try:
        result = await svc.generate_seedance_2_fast_t2v(
...
@pytest.mark.asyncio
async def test_seedance_2_fast_t2v_real_concurrent():
    ...
    print("\n" + "=" * 80)
    print(f"🚀 Seedance 2.0 Fast T2V 真实并发测试，组合数: {len(_REAL_COMBOS)}（均 4s，主要 480p）")
    print("=" * 80)
    ...
    assert len(succeeded) > 0, "所有 Fast T2V 组合均失败"
...
@pytest.mark.asyncio
async def test_seedance_2_fast_t2v_real_smoke():
