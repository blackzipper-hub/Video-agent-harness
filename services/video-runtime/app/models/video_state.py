...
class AudioFileUserInput(BaseModel):
    ...
    suno_clip_id: Optional[str] = Field(default=None, description="Suno generated 时的 clip_id；用于 hybrid 转录直接拉 aligned-lyrics（user upload 时为空，hybrid 会走 Suno upload）")
...
class UserInput(BaseModel):
    ...
    user_option: Optional[UserOption] = Field(
        default=None,
        description="用户选择的工具和配置（含 image_generation_tool、video_generation_tool、lipsync_video_tool、aspect_ratio、resolution、duration 等）。口型镜头使用 lipsync_video_tool（LTX 2.3 / Kling V2 AI Avatar Pro / WAN 2.2 Speech-to-Video / Wan 2.5 / Wan 2.6 Flash）。aspect_ratio/resolution 的实际支持范围依赖所选视频/图像工具；详见 GET /agent-router/options/capabilities。"
    )
    agent_type: Optional[str] = Field(default=None, description="前端指定的代理类型：image/video/music")
...
class CharacterProfiles(BaseModel):
    ...
    def resilience_empty_reason(self) -> Optional[str]:
        ...
        # 每个视频至少要 1 个视觉元素才能往下生成 keyframe；characters 空 = LLM 没干活，应该重试 / 换模型。
        if not self.characters:
            return "characters is empty"
        return None
...
class VideoAnalysisResult(BaseModel):
    ...
    content_category: Optional[ContentCategory] = Field(default=None, description="内容类别：Default / Lip-Sync MV / Product Launch / Short Drama")
    ...
    # Folded into analysis.extra → DB video_analysis.additional_data (JSON)
    # Canonical keys: research, proposal
    extra: Dict[str, Any] = Field(
        default_factory=dict,
        description="含 research / proposal 等扩展；落 DB additional_data",
    )


class ContentCategoryEarlyOutput(BaseModel):
    """user_input 阶段从用户文案推断 content_category（music 前 workflow 路由用）。"""

    content_category: ContentCategory = Field(
        description='内容类别，仅可为 "Default"、"Lip-Sync MV"、"Product Launch" 或 "Short Drama"'
    )
...
class StoryboardScene(BaseModel):
    ...
    title: str = Field(description="场景标题（含记忆点/戏剧功能，如「高潮：文件夹拍胸甲」）")
    description: str = Field(
        description=(
            "场景描述：必须含 NARRATIVE_ROLE / INFORMATION_ROLE / SHOT_INTENT / DIALOGUE / BEATS "
            "以及 SUBJECT/SUBJECT_MOTION/SCENE/SPATIAL/CAMERA；BEATS 为时间码事件链；"
            "DIALOGUE 格式为 角色（情绪）：「台词」或写明无对白"
        )
    )
    duration: float = Field(description="场景时长（秒）")
    camera_angle: str = Field(description="镜头角度与运镜")
    character_action: str = Field(
        description="角色表演与对白：角色（情绪）：「台词」｜可见动作落点"
    )
...
class ScenesCollection(BaseModel):
    ...
    def resilience_empty_reason(self) -> Optional[str]:
        # 被 llm_resilience._wrap_invoke_with_result_check 自动调用：返回非空字符串触发 EMPTY_RESPONSE 桶重试 / 换模型。
        # Gemini 偶发会返回 {"scenes": []}，由于 scenes 字段允许空 list parse 不会报错，这里兜底声明语义为空。
        # 空槽位场景（audio-driven 模式下无 audio_segment 槽位）在调用方已经短路返回，不会走到 LLM 调用，所以这里直接判 `not scenes` 即可。
        if not self.scenes:
...
# ==================== LLM 输出用（Video / Audio 公用）：章节 id/title/description/order；Video 可带 duration
# Video：LLM 先写 chapter.duration；程序仅在 sum≠target / ≤0 等出错时纠正。
# Audio（曲式必有）：程序按 section 1:1 填 duration、audio_section_uuid、audio_segment_ids（覆盖 LLM 秒数）。
# 不拆两套 schema，prompt 与 conversion 分支区分即可。
class StoryChapterForLLMMode(BaseModel):
    """故事章节（LLM 输出）：id/title/description/order；Video 须填 duration；无曲式时可填 audio_segment_indices"""
    id: str = Field(description="章节唯一ID")
    title: str = Field(description="章节标题")
    description: str = Field(description="章节描述")
    order: int = Field(description="章节顺序（0-based，第一章 order=0）")
    duration: Optional[float] = Field(
        default=None,
        description="章节时长（秒）。Video-driven 必填：各章 duration 之和须等于 total_duration；允许长短不均（如对白章更长）。Audio-driven 有曲式时由程序按段落覆盖，可不填。",
    )
...
class StoryOutlineForLLMMode(BaseModel):
    """故事梗概（LLM 输出）：Video/Audio 公用；Video 的 chapter.duration 由 LLM 填写，程序校验后必要时纠正"""
    ...
    def resilience_empty_reason(self) -> Optional[str]:
        # 被 llm_resilience._wrap_invoke_with_result_check 自动调用：返回非空字符串触发 EMPTY_RESPONSE 桶。
        # 大纲是后续所有 node 的基础；structure.chapters 空 = 整个 pipeline 没法继续，必须重试 / 换模型。
        # 标量字段（title/theme/...）虽然 LLM 也可能给空串，但只要 chapters 非空业务还能继续，所以这里只看 chapters。
        if not self.structure or not self.structure.chapters:
            return "structure.chapters is empty"
        return None
...
class DetailedShotLLMOutput(BaseModel):
    ...
    # 4️⃣ 声音设计（Sound Design）— 对白≠旁白 TTS
    dialogue: str = Field(
        description="角色台词（进视频口型/Seedance 引号对白，禁止当作旁白 TTS）"
    )
    sound_effects: str = Field(description="音效")
    narration: Optional[str] = Field(
        default=None,
        description="画外旁白（仅 TTS 旁白时填写；有角色对白时不要把台词写进这里）",
    )
    narration_gender: Optional[Literal["f", "m"]] = Field(
        default=None,
        description="旁白讲解者性别：有 narration 时必填，f=女声 m=男声；无旁白时为 null",
    )
    speaker_directions: Optional[str] = Field(
        default=None,
        description="声画指示，如：silence / Seedance口型对白；旁白静默 / TTS旁白",
    )
    delivery_note: Optional[str] = Field(
        default=None,
        description="交付说明，如：No TTS. Seedance quoted dialogue. / TTS旁白。",
    )
...
class DetailedShot(BaseModel):
    ...
    narration_gender: Optional[str] = Field(
        default=None,
        description="旁白讲解者性别 f/m，由 detail 节点写入 additional_data.narration_gender",
    )
    ...
    # === DB 扩展（per_shot_generation_routing：generation_routing JSONB）===
    generation_routing: Optional[Dict[str, Any]] = Field(
        default=None, description="video_detailed_shots.generation_routing JSONB：per-shot 路由与工具候选"
    )
    additional_data: Optional[Dict[str, Any]] = Field(default=None, description="video_detailed_shots.additional_data JSON")
...
class StoryboardDetailLLMOutput(BaseModel):
    ...
    def resilience_empty_reason(self) -> Optional[str]:
        # 被 llm_resilience._wrap_invoke_with_result_check 自动调用：返回非空字符串触发 EMPTY_RESPONSE 桶。
        # 输入 N 个 scene 必须生成 M 个详细 shot（M>=1）；shots 空 = LLM 漏答，该批 pipeline 必败。
        if not self.shots:
...
class VideoAgentState(TypedDict, total=False):
    ...
    # === 工作流 / 音乐意图（user_input_analysis 后写入，music_generation 复用）===
    music_intent: Optional[str]  # lyrics_provided | auto_lyrics_song | instrumental_bgm
    music_workflow_mode: Optional[str]  # user_upload | suno_then_transcribe | bgm_parallel | none_sora | product_launch | short_drama
    shot_workflow_mode: Optional[str]  # keyframe_i2v | reference_t2v（Seedance2 跳过 keyframe 管线）
    workflow_path: Optional[List[Dict[str, str]]]  # user_input_analysis 后写入，workflow_state.path
    ...
    # === 阶段失败（generation 节点写入，对应 gate 读取后做「失败暂停」并抑制自动继续）===
    # 结构见 services/agent/video/stage_failure.py build_stage_failure()
    #   {stage, blocking, failure_category, total, failed, message_key, message, failed_items}
    # 成功阶段写 None 清除，避免跨 gate 误判。
    stage_failure: Optional[Dict[str, Any]]
...
class VideoGenerationVersion(BaseModel):
    ...
    preview_video_url: Optional[str] = Field(
        default=None, description="lipsync 去音轨前的带声预览 URL（存 additional_data，API 单独字段返回）"
    )
    reference_image_urls: Optional[List[str]] = Field(
        default=None,
        description="T2V/视频生成实际使用的参考图 URL 列表（存 additional_data，API 单独字段返回）",
    )
    raw_error_msg: Optional[str] = Field(default=None, description="原始错误（仅内部/admin，不返前端）")
