...
def _parse_user_option_json(raw: Any) -> Dict[str, Any]:
    if not raw:
        return {}
    try:
...
async def async_merge_conversation_user_option_content_category(
    conversation_id: int,
    content_category: str,
    *,
    run_id: Optional[str] = None,
) -> bool:
    """将 content_category 合并写入 conversations.user_option；可选同步当前 run 快照。"""
    ...
    async with pool.acquire() as conn:
        row = await fetch_one(
            conn,
            "SELECT user_option FROM conversations WHERE id = $1",
            conversation_id,
        )
        if not row:
            return False
        merged = _parse_user_option_json(row.get("user_option"))
        if merged.get("content_category") != content_category:
            merged["content_category"] = content_category
            await execute(
                conn,
                "UPDATE conversations SET user_option = $1, updated_at = $2 WHERE id = $3",
                json.dumps(merged),
                now_utc(),
                conversation_id,
            )
        if run_id:
            run_row = await fetch_one(
                conn,
                "SELECT user_option FROM conversation_runs WHERE run_id = $1",
                run_id,
            )
            if run_row:
                run_merged = _parse_user_option_json(run_row.get("user_option"))
                if run_merged.get("content_category") != content_category:
                    run_merged["content_category"] = content_category
                    await execute(
                        conn,
                        "UPDATE conversation_runs SET user_option = $1, updated_at = $2 WHERE run_id = $3",
                        json.dumps(run_merged),
                        now_utc(),
                        run_id,
                    )
        return True
...
async def async_merge_conversation_run_stage_timing(
    run_id: str, node_name: str, elapsed_sec: float
) -> bool:
    """累加单个 node 的墙钟到 conversation_runs.additional_data.stage_timings[node_name]（秒）。

    不新增 DB 列，写入现有 additional_data（jsonb）。并行 node 同行更新由 row 锁串行化（各自独立语句）。
    additional_data 列不存在（未跑迁移）时静默跳过。
    """
    ...
    async with pool.acquire() as conn:
        ...
        result = await execute(
            ...
            SET additional_data = jsonb_set(
                    ...
                    '{stage_timings}',
                    COALESCE(additional_data->'stage_timings', '{}'::jsonb)
                        || jsonb_build_object($1::text, $2::float),
                    true
                ),
                updated_at = $3
            WHERE run_id = $4
...
def _escape_ilike(term: str) -> str:
    """转义 ILIKE 中的特殊字符（\\ % _），按默认转义符 \\ 处理，避免用户输入被当作通配符。"""
...
async def async_get_user_conversations(
    ...
    支持可选检索：q 按 title / 首条用户输入(user_input) 做 ILIKE 子串匹配（对中文有效），
    start_time/end_time 按 last_active_at 做时间范围过滤，agent_type 精确过滤。
    """
    pool = get_asyncpg_pool()
    async with pool.acquire() as conn:
        # 动态构建 WHERE 条件与参数（参数化，防注入）
        ...
        if q and q.strip():
            ...
            idx = len(params)
            conditions.append(f"(title ILIKE ${idx} OR user_input ILIKE ${idx})")

        if start_time is not None:
            params.append(start_time)
            conditions.append(f"last_active_at >= ${len(params)}")
...
async def async_get_message_event_type_and_data_for_conversation(
    message_id: int,
    conversation_id: int,
) -> Optional[Tuple[Optional[str], Dict[str, Any]]]:
    """按 id + conversation_id 双条件查询（防跨会话误用 message id）；event_data 解析失败或类型异常时回退 {}，与 partial 更新路径一致。"""
    pool = get_asyncpg_pool()
    async with pool.acquire() as conn:
        row = await fetch_one(
            conn,
            "SELECT event_type, event_data FROM conversation_messages WHERE id = $1 AND conversation_id = $2",
            message_id,
            conversation_id,
        )
    if not row:
    ...
    if raw_ed is None:
        ...
        try:
            ed = json.loads(raw_ed) if isinstance(raw_ed, str) else {}
        except Exception:
            ed = {}
    return (et, ed)
...
async def async_get_run_ids_by_thread_id(thread_id: str) -> List[str]:
    """根据 thread_id 获取所有关联的 run_id 列表（按创建时间正序）。"""
    pool = get_asyncpg_pool()
    async with pool.acquire() as conn:
        rows = await fetch_all(
            conn,
        ...
        return [row["run_id"] for row in rows if row and row.get("run_id")]
...
async def async_create_conversation_run_for_regenerate(
    ...
    *,
    ...
    shot_numbers / character_uuids 写入 additional_data，供前端 /conversation/detail 精确 preset 右侧 dice loading。
    返回是否插入成功。
    """
    ...
    if not conversation or conversation.user_id != user_id:
        return False
    additional_data: Optional[Dict[str, Any]] = None
    if shot_numbers or character_uuids:
        additional_data = {}
        if shot_numbers:
            additional_data["shot_numbers"] = sorted({int(s) for s in shot_numbers})
        if character_uuids:
            seen: List[str] = []
            for cu in character_uuids:
                u = str(cu or "").strip()
                if u and u not in seen:
                    seen.append(u)
            if seen:
                additional_data["character_uuids"] = seen
