...
=============================================
与 Cuti-VideoAgent ``chat_agent/services/worker/billing_worker.py`` 同步复制；合并时请以 VideoAgent 为准做 diff。
...
依赖（与 VideoAgent 同构 ``chat_agent``）：``crud.conversation``、``utils.credit_deduction_utils``、
``services.tool_service``、``services.redis.connection``、``schemas.user``、``crud.error_tracking`` 等。
本仓库若未迁入上述模块，请将 VideoAgent 的 ``chat_agent`` 置于 PYTHONPATH 或补齐文件后再运行。

单独运行（本地测试）：
    cd Cuti-VideoChatAgent  # 或 Cuti-VideoAgent
    ENVIRONMENT=local python -m app.chat.services.worker.billing_worker
"""
import asyncio
import logging
...
_AGENT_TYPE_TO_CREDIT: dict[AgentType, CreditOperationType] = {
    ...
    AgentType.VIDEO_GEN: CreditOperationType.VIDEO_GENERATION,
