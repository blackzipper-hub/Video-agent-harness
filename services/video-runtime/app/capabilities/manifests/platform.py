...
def platform_capabilities() -> list[CapabilityManifest]:
    ...
    return [
        ...
        _manifest("atomic.image.generate", "Generate one image directly from the final prompt and explicit references.", "atomic.direct", "image", alias="atomic-image", optional=["text", "image"], parameters_schema={"type": "object", "required": ["prompt"], "properties": {"prompt": {"type": "string", "minLength": 1}, "model": {"type": "string"}, "resolution": {"type": "string", "enum": ["480p", "720p", "1080p"]}, "aspect_ratio": {"type": "string", "enum": ["16:9", "9:16", "1:1"]}, "images": {"type": "array", "items": {"type": "string"}, "description": "Source or setting image URIs or ArtifactVersion ids. Required when an uploaded source locks identity; depends_on is not enough."}, "reference_from_steps": {"type": "array", "items": {"type": "string"}, "description": "Plan step ids whose image artifacts are identity or setting references, including uploaded source-* steps."}, "artifact_title": {"type": "string"}}, "additionalProperties": True}),
            ...
            parameters_schema={
                ...
                "properties": {
                    "user_input": {
                        "type": "string",
                        "description": (
                            "Text brief for web research. Name locked visible subjects "
                            "already read from uploads. This capability does not see images."
                        ),
                    },
