 ...
 * 「我的作品」检索结果项（成片）
 */
export interface CreationSearchItem {
  uuid: string
  thread_id: string
 ...
 * 「我的作品」检索响应
 */
export interface CreationSearchResponse {
  creations: CreationSearchItem[]
  page: number
  size: number
  total: number
}
...
export interface VideoHistory {
  ...
  uploaded_audio_files?: string[]
  source_video_urls?: Record<string, string>
  source_narration_urls?: Record<string, string>
  source_audio_effect_urls?: Record<string, string>
  source_music_urls?: Record<string, string>
  assembly_mode?: string
  title?: string
  folder_id?: number | null
  folder_name?: string | null
  additional_data?: Record<string, LegacyApiValue>
}
...
/** 音乐 MV 三层：音频段落 (Section)，对应 GET /video-analysis/audio-transcription/{uuid} 的 sections[] */
export interface AudioSectionResponse {
  uuid: string
  ...
  section_emotion?: string
  suggested_visual_intensity?: string
  suggested_rhythmic_strategy?: string
  suggested_visual_theme?: string
  suggested_context?: string
}

/** 音乐 MV 三层：音频片段 (Segment)，含 emotion / tempo / vocal_presence */
export interface AudioSegmentDetailResponse {
  uuid: string
  id: number
  start: number
  end: number
...
/** 音频转录详情响应（含整曲 Global + sections + segments），GET /video-analysis/audio-transcription/{uuid} */
export interface AudioTranscriptionDetailResponse {
  task: string
  language: string
  duration: number
  text: string
  audio_url: string
  run_id: string
  segments: AudioSegmentDetailResponse[]
  sections: AudioSectionResponse[]
  created_at: string
  updated_at: string
  words?: Array<{ id: number; word: string; start: number; end: number }>
  ass_subtitle_url?: string
  song_name?: string
  global_bpm?: number
  genre?: string
  global_emotion?: string
  suggested_global_theme?: string
 ...
 * Character History
 */
export interface CharacterMultiViewImageVersion {
  id: string  // 版本UUID
  version_number: number  // 多视角图版本号
...
export interface CharacterVersion {
  ...
  created_at: string
  updated_at: string
  selected_multi_view_version_id?: string  // ⭐ 选中的多视角图版本UUID
  // ⭐ 新结构：多视角图版本对象（可能包含多个版本）
  multi_view_image_version?: CharacterMultiViewImageVersion
  multi_view_versions?: CharacterMultiViewImageVersion[]  // ⭐ 所有多视角图版本列表
}
...
export interface CharacterHistory {
  ...
  selected_version_id?: string  // ⭐ 选中的角色版本UUID
...
export interface MediaHistoryV1PagedResponse {
  code: number
  data: {
    page: number
    limit: number
