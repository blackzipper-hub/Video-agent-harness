...
export default function StudioWorkspacePage() {
  ...
  return <main className="min-h-screen bg-zinc-950 text-zinc-100">
    ...
    <div className="grid gap-4 p-5 lg:grid-cols-[1.1fr_1fr_1fr]">
      ...
      <Panel title={`Skill · ${skills.length}`} empty={copy.skillEmpty}>{skills.map((skill, index) => <div key={String(skill.id || index)} className="rounded-lg border border-white/10 p-3 text-sm"><Sparkles className="mr-2 inline h-4 w-4 text-violet-300" />{String(skill.name || skill.id || 'Skill')}</div>)}</Panel>
