...
export function VideoOptionsPanel({
  ...
  const lipsyncSelectOptions = useMemo(() => {
    const raw =
  ...
  useEffect(() => {
    if (resolutionOptions.length && !resolutionOptions.some((r) => r.value === resolution)) {
      onResolutionChange(resolutionOptions[0].value as "480p" | "720p" | "1080p");
    }
  ...
  useEffect(() => {
    if (aspectRatioOptions.length && !aspectRatioOptions.some((r) => r.value === aspectRatio)) {
      onAspectRatioChange(aspectRatioOptions[0].value as "16:9" | "1:1" | "9:16");
    }
      ...
      >
        ...
        <div className="space-y-2 py-4">
          ...
          <Slider
            ...
            onValueChange={(v) => onLipsyncCoverageChange(v[0])}
            min={0}
            max={100}
            step={1}
            thumbStyle="minimal"
