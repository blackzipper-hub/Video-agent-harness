      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            {!hideBackButton && (
              <Button variant="ghost" size="icon" className="w-8 h-8 text-white hover:bg-white/10" onClick={onBack} aria-label={t('back' as any) || 'Back'}>
                <ArrowLeft className="w-4 h-4" />
              </Button>
            )}
            <span className="text-sm text-white/70">{splitTiles.length} {t('imagesSplit' as any)}</span>
          </div>
          <Button size="sm" className="bg-white/20 text-white border-white/30 hover:bg-white/30" onClick={async () => { const base = imageTitle.replace(/\s+/g, '_'); await downloadAllTiles(splitTiles, base); }}>
            <Download className="w-4 h-4 mr-2" />{t('downloadAll' as any)}
          </Button>
        </div>
    ...
    <>
      <div className="space-y-4 p-4 rounded-lg border border-white/20 bg-white/5">
        <div className="flex items-center gap-2 mb-2">
          {!hideBackButton && (
            <Button variant="ghost" size="icon" className="w-8 h-8 text-white hover:bg-white/10 shrink-0" onClick={onBack} aria-label={t('back' as any) || 'Back'}>
              <ArrowLeft className="w-4 h-4" />
            </Button>
          )}
        ...
        <Button className="w-full bg-white text-black hover:bg-white/90" onClick={onSplit} disabled={isSplitting}>
          {isSplitting ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />{t('splitting' as any)}</> : <><Scissors className="w-4 h-4 mr-2" />{t('splitNow' as any)}</>}
        </Button>
      </div>
