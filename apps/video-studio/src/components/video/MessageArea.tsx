...
export const MessageArea = ({
  ...
  const assistantThinkingText = language === "zh" ? "Cuti 正在思考..." : "Cuti is thinking...";
  ...
  // 固定为第二张图的小尺寸：空或单行时始终 40px，多行时最高 200px
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                {language === "zh" ? "描述你的创作想法，开始新的任务…" : "Start by describing your video idea..."}
              </p>
            </div>
          </div>
        )}
      ...
      >
        ...
        {isInputDisabled && !effectiveIsGenerating ? (
          ...
          <>
            ...
            {/* 拖拽上传提示覆盖层 */}
            {isDragOver && (
              <div className="absolute inset-0 bg-primary/10 border-2 border-dashed border-primary/50 rounded-full flex items-center justify-center z-10">
                <div className="text-center">
            ...
            {/* Pill 输入条：边缘流光 + 紫色描边；手机端两行布局，桌面端单行 */}
            <div
              ref={pillWrapperRef}
              className="relative flex rounded-full bg-white p-1.5 pl-5 dark:bg-neutral-900 [box-shadow:0_0_0_1.5px_var(--chat-brand-from),0_8px_24px_oklch(0_0_0_/0.06)]"
              onMouseMove={(e) => {
            ...
            >
              <div ref={spotlightRef} className="pointer-events-none absolute inset-0 rounded-full" aria-hidden />
              <div className="flex min-w-0 flex-1 flex-col gap-2 overflow-hidden rounded-full bg-white pr-0 dark:bg-neutral-900 sm:flex-row sm:items-center">
          <input
            ref={fileInputRef}
            type="file"
            multiple
          ...
          />
          {/* 第一行（手机）/ 左侧（桌面）：用户输入或 prompt tag */}
          <div className={`flex-1 min-w-0 w-full flex items-center relative ${isInputDisabled ? 'cursor-not-allowed' : ''}`}>
            {(() => {
              ...
              return (
                <>
                  <Textarea
                    ...
                    className={`w-full border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 font-inter resize-none min-h-[40px] max-sm:min-h-[21px] max-h-[200px] py-2 max-sm:py-0.5 text-[14px] max-sm:text-[12px] leading-tight text-foreground placeholder:text-foreground/40 max-sm:placeholder:text-[12px] h-10 max-sm:h-[21px] ${
          ...
          {/* 第二行（手机）/ 右侧（桌面）：手机端上传在左、选项+发送在右 */}
          <div className="flex items-center gap-2 flex-shrink-0 max-sm:justify-between max-sm:w-full">
