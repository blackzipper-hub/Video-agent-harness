...
The built-in `cuti.seedance-story` workflow creates script, character, storyboard, reference image, keyframe, clip, extracted tail-frame, timeline, narration, BGM, subtitle, and final-video artifacts. Each successful step is saved as a draft and can be reused after restart. Clips are dependency-serialized: each real tail frame becomes the strict start-frame input of the following shot. The active project version changes only after all media and validation steps pass.
