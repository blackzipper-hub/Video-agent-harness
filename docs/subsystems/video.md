...
`ctx.videoRuntime` is the project API consumed by video tools. DeepSeek owns the agent loop; the Python Runtime owns durable project versions, artifact dependencies, builds, and planning checkpoints. The [service package](../../packages/video/video-runtime/README.md) defines consumer expectations and the [HTTP provider](../../packages/video/video-runtime-http/README.md) defines transport configuration.

## Request and response types

The [TypeScript definitions](../../packages/video/video-runtime/src/types.ts) are the source of truth for serializable fields. `RequestIdentity` supplies user/session attribution, not permissions. `ProjectSnapshot` and `BuildSnapshot` describe durable state. `BuildPlanRequest`, `RebuildRequest`, `ArtifactSelectionRequest`, and `ExportRequest` carry idempotency and base-version values; stale edits are rejected rather than applied to a different version.

`WorkflowSummary`, `WorkflowDetail`, `SkillDetail`, and `SkillResourceDetail` expose installed instructions, resources, and availability. `ChangePreview` classifies dependency impact; `BuildPlanSnapshot` contains persisted steps, costs, and planning revisions. `MediaArtifactSummary` and `CapabilityPromptView` are planner discovery results. `PlanPatchPreviewRequest` binds operations to artifact inputs rather than inventing file URLs. `PlanCheckpoint` and `CheckpointResolutionRequest` coordinate revision-checked continuation in the same Session.

## Cancellation and durability

Aborting an HTTP request does not undo accepted work. Build cancellation is explicit; submitted upstream jobs may continue and incur costs. Preview operations do not submit paid generation. Follow the [Runtime API and scheduling reference](../../services/video-runtime/README.md) for execution and recovery semantics.
