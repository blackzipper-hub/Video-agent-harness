...
本地项目和构建状态复用已有的可崩溃恢复 `LocalJsonVideoProjectRepository`；生成媒体和沙箱工作目录也保存在每用户数据目录中。CLI 只暴露一个 Video Studio 端口，并将产品 API 路径代理到 Video Runtime。生产、多用户、PostgreSQL 和不受信任插件隔离仍可使用 Docker Compose。
...
用户可以运行 `npx @cuti-ai/video-agent-harness web`，无需安装 Docker、系统 Python、PostgreSQL 或系统 FFmpeg。首次启动需要下载 Python 并安装 Python wheel。生产部署继续保留现有 Docker 和 PostgreSQL 路径。
